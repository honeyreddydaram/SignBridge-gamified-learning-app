...
- `generate_vocabulary_exercises` (vocabulary): built from `VOCABULARY_CATEGORIES` (10 thematic groups covering all 114 words in `ml/word_signs_verified.json`, validated at call time by `get_vocabulary_categories()` — it raises loudly if a category references a word missing from the manifest, rather than silently generating a broken lesson). Per word: a `vocab_video_card` (video + description + attribution) then a `vocab_comprehension_mcq` ("what does this sign mean?", 3 random distractor words). Ends with one `vocab_mirror_practice` per lesson (not per word) — see below for why this isn't a `camera_challenge`.

**Why vocabulary lessons have no recognition-graded camera challenge**: the trained recognition model (`ml/models/asl_landmark_classifier.joblib`) only classifies static A-Z fingerspelling handshapes from a single frame — it was never trained on, and structurally cannot validate, dynamic multi-frame word-level signs like HELLO or THANK YOU. Claiming to auto-grade those would be exactly the fabricated-working-feature this project rules out elsewhere. `vocab_mirror_practice` (`frontend/src/components/VocabMirrorPractice.tsx`) instead gives the learner their own camera feed alongside a reminder of the words just covered, explicitly ungraded — self-directed practice, not a recognition claim.
